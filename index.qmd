...
def fig_antall_med_glidende_gjennomsnitt(tittel, timestamp_felt, tabell):
    ...
    fig.add_trace(go.Bar(x=df["date"], y=df["count"], name="Antall per dag"))
...
value = antall_for_n_uker_siden(
    2, "vedtak_fattet", "styringsinfo_dataset.styringsinfo_vedtak_fattet_view"
)
...
value = antall_for_n_uker_siden(
    1, "vedtak_fattet", "styringsinfo_dataset.styringsinfo_vedtak_fattet_view"
)
print(f"{value}\navklart forrige uke")
```
...
value = antall_for_n_uker_siden(
    0, "vedtak_fattet", "styringsinfo_dataset.styringsinfo_vedtak_fattet_view"
)
print(f"{value}\navklart denne uken")
```
...
QUERY = f"""
    ...
    GROUP BY year, month, har_utbetaling
...
df_months_med_utbetaling = df_months[df_months.har_utbetaling == True]
df_months_uten_utbetaling = df_months[df_months.har_utbetaling == False]
...
fig.add_trace(
    go.Bar(
        x=[
            ...
            for _, row in df_months_med_utbetaling.iterrows()
        ],
...
fig.add_trace(
    go.Bar(
        x=[
            f"{row['year']}-{row['month']:02}"
            for _, row in df_months_uten_utbetaling.iterrows()
        ],
...
- For behandlingstid mellom 1 og 90 dager viser grafen under fordelingen
- For vedtak med flere søknader bruker vi behandlingstiden på den søknaden som ble sendt inn sist
