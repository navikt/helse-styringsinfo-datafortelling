...
def antall_for_n_uker_siden(antall_uker_siden, timestamp_felt, tabell):
    ...
    AND DATE({feltimestamp_felt}, 'Europe/Oslo') <= DATE_SUB(DATE_ADD(DATE(CURRENT_TIMESTAMP(), 'Europe/Oslo'), INTERVAL -EXTRACT(DAYOFWEEK FROM CURRENT_TIMESTAMP()) + 8 DAY), INTERVAL 7 * {str(antall_uker_siden)} DAY)
    """
