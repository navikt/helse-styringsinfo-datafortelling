...
<div class="flexjarcontainer">
    ...
    <p style="font-size: 16px">Fant du den informasjonen du trengte?</p>
    <div class="buttons">
...
<script>
document.getElementById('yesButton').addEventListener('click', showAdditionalContent);
document.getElementById('noButton').addEventListener('click', showAdditionalContent);
...
function showAdditionalContent() {
    ...
    additionalContent.innerHTML = `
        ...
        <div id="errorMessage">Det har oppstått en feil</div>
        </div>
    `;
    ...
    .then(response => {
        if (!response.ok) {
            throw new Error('Meldingen ble ikke sendt på grunn av tilkoblingsfeil mot serveren. Prøv igjen nå eller senere.');
        }
        return response.json();
    })

    .then(data => {
        var flexjarcontainer = document.querySelector('.flexjarcontainer');
        flexjarcontainer.style.display = 'none';
        var thankYouMessage = document.createElement('p');
        thankYouMessage.textContent = 'Takk for tilbakemeldingen din.';
        additionalContent.appendChild(thankYouMessage);
    })

    .catch(error => {
    console.error('Error:', error);
    var errorMessageDiv = document.getElementById('errorMessage');
    errorMessageDiv.textContent = 'Det har oppstått en feil';
