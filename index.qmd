---
title: "Styringsdata: produktområde helse"
...
Denne datafortellingen har som hensikt å formidle data som kan gi grunnlag for bedre og mer presise prioriteringer knyttet til behov. Behovene vi ønsker å bistå med er i hovedsak rettet mot saksbehandling (i ny sykepengeløsning) og for overordnede strategiske valg.  
...
WITH agg as (
    ...
    agg_buckets AS (
    SELECT SUM(antall_vedtak) AS sum_vedtak, '1 - alle' AS vindu from agg
    UNION ALL
    select SUM(antall_vedtak), '2 - under én dag' from agg WHERE dager_brukt = 0
    UNION ALL
    select SUM(antall_vedtak), '3 - én dag til én uke' from agg WHERE dager_brukt > 0 AND dager_brukt <7
    UNION ALL
    select SUM(antall_vedtak), '4 - én til fire uker' from agg WHERE dager_brukt >= 7 AND dager_brukt < 28
    UNION ALL
    select SUM(antall_vedtak), '5 - fire til åtte uker' from agg WHERE dager_brukt >= 28 AND dager_brukt < 56
    UNION ALL
    SELECT SUM(antall_vedtak), '6 - åtte uker eller mer' FROM agg WHERE dager_brukt >= 56
)
SELECT sum_vedtak, vindu FROM agg_buckets ORDER BY vindu;
"""
